...
/**
 * @brief Initializes the fixed point math functions. Needs to be called once at startup
 *
 * Fills the LUTs for interpolation (sin, cos, atan/atan2, acos, sqrt) and prepares the interpolator configs
 */
void initFixMath();

/**
 * @brief Loads the interpolator configs for fixed point math
 *
 * Call this once before every fixed point math calculation batch (trigonimetric functions and sqrt), i.e. every time before you start a bunch of sinFix/cosFix/whatever.
 * Whether these are sin, sqrt or any other doesn't matter. Not needed for basic operations such as +, -, *, /, << etc.
 */
...
/**
 * @brief Calculates the sine of a fixed point number
 *
 * Accurate to about 0.0001.
 * Important: Call initFixMath() once at the start and don't forget startFixMath()!
 * @param x radians
 * @return fix32
 */
...
/**
 * @brief Calculates the cosine of a fixed point number
 *
 * Accurate to about 0.0001.
 * Important: Call initFixMath() once at the start and don't forget startFixMath()!
 * @param x radians
...
/**
 * @brief Calculates the sine and cosine of a fixed point number
 *
 * Accurate to about 0.0001. The main point of this function is that it is even faster than sinFix and cosFix by reducing the parts they have in common.
 * Important: Call initFixMath() once at the start and don't forget startFixMath()!
 * @param x radians
...
/**
 * @brief Calculates the atan of a fixed point number
 *
 * Accurate to about +-0.00003.
 * Important: Call initFixMath() once at the start and don't forget startFixMath()!
 *
 * @param x input
 * @return fix32 radians (-pi / 2 ... +pi / 2)
 */
fix32 atanFix(fix32 x);

/**
 * @brief Calculates the atan2 of a fixed point number
 *
 * Accurate to about +-0.00003.
 * Important: Call initFixMath() once at the start and don't forget startFixMath()!
 *
 * @param x input
 * @return fix32 radians (-pi ... +pi)
 */
 ...
 *
 * @param x input (-1 ... 1)
 * @return fix32 radians (0 ... +pi), +pi or 0 if out of range
 */
 ...
 *
 * Resolution of the output is about 17 bits (lower if output is < 1)
 * Important: Call initFixMath() once at the start and don't forget startFixMath()!
 *
 * @param x input
 * @return fix32 output
...
// cmath costants
// adding 0.5 / 65536 for rounding

constexpr fix32 FIX_PI = M_PI + 0.5 / 65536;
constexpr fix32 FIX_PI_2 = M_PI_2 + 0.5 / 65536;
constexpr fix32 FIX_PI_4 = M_PI_4 + 0.5 / 65536;
constexpr fix32 FIX_3PI_4 = M_3PI_4 + 0.5 / 65536;
constexpr fix32 FIX_2_PI = M_2_PI + 0.5 / 65536;
constexpr fix32 FIX_1_PI = M_1_PI + 0.5 / 65536;
constexpr fix32 FIX_TWOPI = M_TWOPI + 0.5 / 65536;
constexpr fix32 FIX_SQRTPI = M_SQRTPI + 0.5 / 65536;
constexpr fix32 FIX_SQRT2 = M_SQRT2 + 0.5 / 65536;
constexpr fix32 FIX_SQRT1_2 = M_SQRT1_2 + 0.5 / 65536;
constexpr fix32 FIX_E = M_E + 0.5 / 65536;
constexpr fix32 FIX_LOG2E = M_LOG2E + 0.5 / 65536;
constexpr fix32 FIX_LOG10E = M_LOG10E + 0.5 / 65536;
constexpr fix32 FIX_LN2 = M_LN2 + 0.5 / 65536;
constexpr fix32 FIX_LN10 = M_LN10 + 0.5 / 65536;

constexpr fix32 FIX_RAD_TO_DEG = 180 / M_PI + 0.5 / 65536;
constexpr fix32 FIX_DEG_TO_RAD = M_PI / 180 + 0.5 / 65536;
constexpr fix32 FIX32_MIN = fix32().setRaw(0x80000000);
constexpr fix32 FIX32_MAX = fix32().setRaw(0x7FFFFFFF);
constexpr fix64 FIX64_MIN = fix64().setRaw(0x8000000000000000LL);
constexpr fix64 FIX64_MAX = fix64().setRaw(0x7FFFFFFFFFFFFFFFLL);
